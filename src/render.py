...
def render_main() -> None:
    ...
    with imageTab:
        ...
        threshold: int = st.slider(
            '请选择警示阈值：', 0, 100, 10, key='state_threshold')
        ...
        if st.session_state['predict_digits'] is not None:
            for predict_digit in st.session_state['predict_digits']:
                if predict_digit >= threshold:
                    ...
                    st.error(
                        f'当前窗口人数为`{predict_digit}`人，已超过 `{threshold}` 人，系统进行人数预警，请注意！', icon="⚠️")
                    ...
                    st.metric(label="当前预测数值", value=predict_digit, delta=bias)
                    st.success(
                        f'当前窗口人数为`{predict_digit}`人，未超过 `{threshold}` 人，请继续保持', icon="👍")
